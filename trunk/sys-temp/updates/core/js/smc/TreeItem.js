...
var TreeItem = function(_oControl, _oParent, _oData, _oSiblingItem, _sInsertMode) {
	...
	*/
	var __draw = function(_oContainerEl) {
		...
		if (!__self.isActive) {
			jQuery(itemIcon).fadeTo(1, 0.5);
			jQuery(labelText).fadeTo(1, 0.5);
			jQuery(toggleCtrl).fadeTo(1, 0.5);
		}
	...
	*/
	this.expand = function() {
		...
		if (!this.loaded) {
			if(CountChilds > maxChildsCount && !this.control.disableTooManyChildsNotification) {
				var self = this;
				openDialog({
