...
JS;
		...
		public function insert($sPlace = "", $iMacrosID=0, $bList = false, $current_element_id = false) {
					...
					if (!in_array($currentPageId, $oNextBanner->getValue('not_view_pages'))) {
						...
						if ($oNextBanner->getValue('max_views') <= 0 || $oNextBanner->getValue('views_count') <= $oNextBanner->getValue('max_views')) {
							...
							if (count($arrBannerTags)) {
								...
								if (!count($arrCommonTags)) {
									$bShowActual = false;
								} else {
									$weight += count($arrCommonTags);
								}
							}
							// do show till filter
							...
							if ($bShowActual) {
								...
								if ($bShowActual) {
									...
									if (is_array($arrBannerTags) && count($arrBannerTags)) {
										if ($oStat = cmsController::getInstance()->getModule("stat")) {		
											...
											if ($referer && $referer == getServer('HTTP_REFERER') && !regedit::getInstance()->getVal("//modules/banners/not-use-referer-tags")) {
												$refererTags = $this->getRefererTags($referer);
											}
											...
											for ($nI=0; $nI < count($arrBannerTags); $nI++) {
												...
												if (in_array($sNextTag, $refererTags)) {
													$iAllowTags +=5;
												}
											}
											if (!$iAllowTags) {
								...
								if ($bShowActual) {
									$arrBannersList[$iNextBanId] = $weight;
								}
							} else {
			...
			if (count($arrBannersList)) {
				...
				arsort($arrBannersList);
				$arrBannersList = array_keys($arrBannersList);		
