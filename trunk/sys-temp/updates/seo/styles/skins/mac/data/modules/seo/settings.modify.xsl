...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/result[@method = 'config']//group[@name = 'yandex']" mode="settings-modify">
		<xsl:variable name="yandexToken" select="option[@name = 'token']" />
		<xsl:variable name="yandexCode" select="option[@name = 'code']" />
			<table class="tableContent">
				<tbody>
					<xsl:choose>
						<xsl:when test="$yandexToken/value != ''">
							<tr>
								<td class="eq-col">
									<label for="{$yandexToken/@name}">
										<xsl:value-of select="$yandexToken/@label" />
									</label>
								</td>
								<td>
									<input type="text" name="{$yandexToken/@name}" value="{$yandexToken/value}" id="{$yandexToken/@name}" />
								</td>
							</tr>
						</xsl:when>
						<xsl:otherwise>
							<tr>
								<td class="eq-col" style="vertical-align: top;">
									<label for="{$yandexCode/@name}">
										<xsl:value-of select="$yandexCode/@label" />
									</label>
								</td>
								<td>
									<input type="text" name="{$yandexCode/@name}" id="{$yandexCode/@name}" />
								</td>
