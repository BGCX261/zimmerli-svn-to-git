	...
	<xsl:template match="properties/group" mode="form-modify">
		<xsl:variable name="label_name">
			<xsl:choose>
				...
				<xsl:when test="../../@type-guid='dispatches-subscriber'"><xsl:text>&label-email-name;</xsl:text></xsl:when>
				<xsl:otherwise><xsl:text>&label-name;</xsl:text></xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:if test="($method = 'edit') and (position() = 1) and (../../@type-guid = 'dispatches-dispatch')">
