...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="ui-smc-table">
		...
		<xsl:param name="label_first_column"></xsl:param>
		...
		<script	type="text/javascript">
			...
			$(document).ready(function() {
				...
				<xsl:if test="$search-show = 1">
					new	filterController(oTable, 'catalog-object', true, $('#search')[0], {
		...
		<div class="paging">&label-elements-per-page;<span id="per_page_limit" /></div>
	</xsl:template>

	<xsl:template match="domain" mode="domain-select">
		<xsl:if test="@id = $domain-id">
			<option value="{@id}" selected="selected"><xsl:value-of select="@host"/></option>
		</xsl:if>
		<xsl:if test="@id != $domain-id">
	...
	<xsl:template name="ui-smc-tree">
		...
		<ul id="tree_container_{$control-id}" class="tree-container allowContextMenu" oncontextmenu="return false">
	...
	<xsl:template name="std-save-button">
		<div class="buttons">
			<div>
				<input type="submit" value="&label-save;" class="{/result/@module}_{/result/@method}_btn" name="save-mode" />
				<span class="l" /><span class="r" />
			</div>
		</div>
	</xsl:template>
	...
	<xsl:template name="std-form-is-active">
		...
		<div class="field" style="height:auto; min-height:25px;">
			<label for="is-active" class="is-active">
				<xsl:if test="/result/data/page/@id">
					<script type="text/javascript">
						<![CDATA[
						jQuery(document).ready(function(){
							jQuery.getScript('//yandex.st/share/share.js', function() {
								new Ya.share({
									...
									'link': 'http://]]><xsl:value-of select="$domain" />/~/<xsl:value-of select="$pid" /><![CDATA[',
									'title': ']]><xsl:value-of select="document(concat('upage://', $pid))//name" /><![CDATA[',
									'popupStyle': {
										'copyPasteField': true
									}
								});
					...
					<div class="share">
						<a href="#" class="switch" onclick="return false;">&raquo;</a>
					</div>
	...
	<xsl:template name="std-form-data-type">
		...
		<xsl:choose>
				...
				<div class="field">
					<label>
						<span class="label">
							...
							<a href="/admin/data/type_edit/{$value}" id="edit">Редактировать тип данных</a>
						</span>
						<span>
							<select name="type-id" class="edit" onchange="changeEditLink();">
			...
			<xsl:otherwise>
				<div class="field">
					<label>
						<span class="label_edit">
							<a href="/admin/data/type_edit/{$value}" id="edit" name="edit">Редактировать тип данных</a>
						</span>
						<span>
							<input type="text" disabled="disabled" value="{$object-types//item}" />
						</span>
					</label>
				</div>
			</xsl:otherwise>
