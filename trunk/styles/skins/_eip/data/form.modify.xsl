...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="data[@type = 'form']//field">
		<p class="error">
			<xsl:text>В данном окне управление некоторыми полями недоступно, поэтому они здесь не показаны.</xsl:text>
		</p>
	</xsl:template>
	...
	<xsl:template match="data[@type = 'form']//field[@type = 'optioned']">
		<p class="error">
			<xsl:text>В данном окне управление опционными свойствами недоступно, поэтому они здесь не показаны.</xsl:text>
		</p>
	</xsl:template>
