...
function filter() {
	...
	*/
	...
	this.Page    = null;
	this.Limit = null;
	this.hTypes   = new Array();
	this.oTypes  = new Array();
	/**
	* @param _Value
	*/
	...
	this.getQueryString = function(_MergeFilter) {
		...
		if(_MergeFilter instanceof Object) {
			...
			for(i=0; i<_MergeFilter.oTypes.length; i++) {
				QueryPieces.push( 'object_types[]=' + _MergeFilter.oTypes[i] );
			}
			o = new Object();
			...
			if(this.Limit  !== null) QueryPieces.push('per_page_limit='  + this.Limit);
