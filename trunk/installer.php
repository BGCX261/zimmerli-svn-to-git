	...
	class umiInstallExecutor {
		...
		*/
		private function getDummyInfo() {
			...
			$ht['allow_array'] = array("install.php", "installer.php", "smu/install.php", "smu/installer.php", "umi_smt.php");
			return $ht;
		}
		...
		*/
		private function ht_create_dummy($fulldummyname) {
			...
			$dummy = $downloader->get_file(base64_decode("aHR0cDovL3d3dy5pbnN0YWxsLnVtaS1jbXMucnUvZmlsZXMvZHVtbXkuaHRtbA=="));
			...
			if (!$this->install_mode && $values!='') { // GUI - обновление
				...
				elseif (!is_null($this->getParam("sv_login"))) {
					...
					* :socket
					...
					$settings['DB']['host'] = trim($host);
					$settings['DB']['port'] = trim($port);
	...
	class umiUpdateDownloader {
		...
		*/
		...
		private function build_query($type, $params = array()) {
			...
			return base64_decode('aHR0cDovL3VwZGF0ZXMudW1pLWNtcy5ydS91cGRhdGVzZXJ2ZXIv') . "?" . http_build_query($params, '', '&');
		}
